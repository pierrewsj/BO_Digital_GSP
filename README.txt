ATUALIZAÇÃO v31.2 — TEMPO DE ELABORAÇÃO X ATENDIMENTO

- O indicador antigo “Em aberto/Duração” foi separado em dois conceitos.
- “Elaboração” mede do momento em que o rascunho é criado até a finalização do BO.
- “Atendimento” só começa quando o vigilante toca em “Iniciar atendimento”.
- O atendimento pode ser encerrado manualmente; se ainda estiver ativo ao finalizar o BO, é encerrado automaticamente naquele horário e a auditoria registra isso.
- As cores normal/atenção/crítico passam a considerar somente atendimento operacional em andamento.
- Rascunhos antigos não são mais interpretados como atendimento crítico apenas por estarem há muito tempo em elaboração.
- Na tela do BO finalizado aparecem separadamente: ocorrência informada, criação do BO, finalização, tempo de elaboração e tempo de atendimento.
- Compatível com Google Apps Script/API v6.3.0; não é necessário alterar o backend para usar esta melhoria.

CORREÇÃO v31.1 — FILA DE SINCRONIZAÇÃO

- Rascunhos permanecem com status “Salvo no aparelho” durante o preenchimento.
- Selecionar referência, preencher campos ou avançar etapas não cria alerta de sincronização.
- O banner “Há registros aguardando sincronização” só considera BOs FINALIZADOS cuja tentativa de envio não foi confirmada, além de conflitos reais.
- Rascunhos marcados indevidamente como pendentes pela v31.0 são corrigidos automaticamente ao abrir a v31.1.
- A fila e o retry automático processam somente BOs finalizados.
