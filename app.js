...
function operationalTimeInfo(record) {
  ...
  const start = new Date(record?.createdAt || record?.updatedAt || Date.now()).getTime();
  const end = record?.status === 'Finalizado' && record?.finalizedAt ? new Date(record.finalizedAt).getTime() : Date.now();
  const minutes = Math.max(0, Math.round((end - start) / 60000));
  const level = record?.status === 'Finalizado' ? 'done' : minutes >= settings.criticalMinutes ? 'critical' : minutes >= settings.warningMinutes ? 'warning' : 'ok';
  const label = minutes < 60 ? `${minutes} min` : `${Math.floor(minutes/60)}h ${minutes%60}min`;
  return { minutes, level, label, warningMinutes:settings.warningMinutes, criticalMinutes:settings.criticalMinutes };
}
...
function normalizeRecord(record) {
  ...
  normalized.schemaVersion = Number(normalized.schemaVersion || 4);
  normalized.basic ||= {};
...
function pendingSyncRecords() { return state.records.filter(record => ['pending','error','local','conflict'].includes(record.syncStatus)); }
...
async function processSyncQueue({ force=false, onlyId='' } = {}) {
  ...
  try {
    await refreshRecords();
    const queue = state.records.filter(record => (!onlyId || record.id === onlyId) && ['pending','error','local'].includes(record.syncStatus) && dueForRetry(record, force));
...
function scheduleSyncRetry() {
  ...
  const candidates = state.records.filter(record => ['pending','error','local'].includes(record.syncStatus));
...
function renderAbout() {
  app.innerHTML = `
    ...
    <section class="info-card"><p class="eyebrow">Tempo operacional</p><h2>Indicador visual de atenção</h2><p>Este indicador é local e configurável; não representa um SLA corporativo oficial. Ele destaca rascunhos pelo tempo em aberto.</p><div class="form-grid"><div class="field"><label for="sla-warning">Atenção a partir de (min)</label><input id="sla-warning" type="number" min="1" max="1440" value="${operationalTimeSettings().warningMinutes}"></div><div class="field"><label for="sla-critical">Crítico a partir de (min)</label><input id="sla-critical" type="number" min="2" max="1440" value="${operationalTimeSettings().criticalMinutes}"></div></div><div class="about-actions"><button class="button secondary" type="button" data-about-action="save-sla">Salvar tempos</button><button class="button secondary" type="button" data-action="show-syncqueue">Ver fila de sincronização</button></div></section>
