...
function renderOperationalTimingCard(record = state.current) {
  ...
  const elaboration = elaborationTimeInfo(record);
  const isDraft = record?.status !== 'Finalizado';
  const startedText = timing.startedAt ? formatDateTime(timing.startedAt) : 'Não registrado';
  const endedText = timing.endedAt ? formatDateTime(timing.endedAt) : timing.ongoing ? 'Em andamento' : 'Não registrado';
  const attendanceChip = timing.active
    ? `<span class="chip time-chip time-${timing.level}">Atendimento: ${escapeHtml(timing.label)}</span>`
    : '<span class="chip time-chip time-muted">Atendimento: não iniciado</span>';
  const actions = isDraft
    ? !timing.active
      ? '<button class="button secondary" type="button" data-timing-action="start">Iniciar atendimento</button>'
      : timing.ongoing
        ? '<button class="button warning" type="button" data-timing-action="end">Encerrar atendimento</button>'
        : ''
    : '';
  return `<section class="info-card operational-timing-card"><div class="operational-timing-head"><div><p class="eyebrow">Controle de tempo</p><h2>Elaboração x atendimento</h2></div><div class="entity-meta"><span class="chip time-chip time-elaboration">Elaboração: ${escapeHtml(elaboration.label)}</span>${attendanceChip}</div></div><p>O tempo de <strong>elaboração</strong> mede quanto tempo o BO ficou em preenchimento. O tempo de <strong>atendimento</strong> só começa quando o vigilante registra o início operacional.</p><dl class="definition-grid timing-definition"><div><dt>Início do atendimento</dt><dd>${escapeHtml(startedText)}</dd></div><div><dt>Fim do atendimento</dt><dd>${escapeHtml(endedText)}</dd></div></dl>${actions ? `<div class="about-actions">${actions}</div>` : ''}</section>`;
}
...
function assistantChecklist(record = state.current) {
  ...
  add('bo-date', 'Data, hora e referência', b.data && b.hora && b.referencia, 'required', 0, 'Informe data, hora e referência da ocorrência.');
  add('bo-name', 'Solicitante identificado', b.nomeEmissor && /^\d+$/.test(b.matriculaEmissor || ''), 'required', 0, 'Informe nome e matrícula do solicitante.');
  add('bo-local-detail', 'Local detalhado', b.local && meaningfulText(b.complementoLocal), 'required', 0, 'Identifique o local com detalhes suficientes.');
  add('', 'Pessoas e testemunhas verificadas', v.people !== 'pending' && v.witnesses !== 'pending', 'required', 1, 'Confirme pessoas e testemunhas relacionadas.', 'people');
  add('', 'Veículos verificados', v.vehicles !== 'pending', 'required', 1, 'Confirme se existe veículo relacionado.', 'vehicles');
  add('', 'Materiais verificados', v.materials !== 'pending', 'required', 1, 'Confirme se existe material relacionado.', 'materials');

  if (context.vehicleRequired) {
    add('', 'Veículo da ocorrência cadastrado', (r.vehicles || []).length > 0, 'required', 1, 'Cadastre ao menos um veículo para esta ocorrência.', 'vehicles');
  } else if (context.vehicle || (r.vehicles || []).length) {
    add('', 'Dados do veículo disponíveis', (r.vehicles || []).length > 0, 'recommended', 1, 'Cadastre placa ou chassi do veículo relacionado.', 'vehicles');
  }

  if ((r.vehicles || []).length) {
    add('', 'Marca, modelo ou empresa do veículo', r.vehicles.every(item => meaningfulText(item.marca) || meaningfulText(item.modelo) || meaningfulText(item.empresa)), 'recommended', 1, 'Complete marca, modelo ou empresa dos veículos quando a informação estiver disponível.', 'vehicles');
  }

  if (context.materialRequired) {
    add('', 'Material, peça ou equipamento cadastrado', (r.materials || []).length > 0, 'required', 1, 'Cadastre o material, peça ou equipamento relacionado.', 'materials');
  } else if (context.material || (r.materials || []).length) {
    add('', 'Material relacionado disponível', (r.materials || []).length > 0, 'recommended', 1, 'Cadastre o material relacionado quando aplicável.', 'materials');
  }

  if ((r.materials || []).length) {
    add('', 'Denominação e quantidade dos materiais', r.materials.every(item => meaningfulText(item.denominacao) && Number(item.quantidade) > 0 && meaningfulText(item.unidade)), 'required', 1, 'Complete denominação, quantidade e unidade dos materiais.', 'materials');
    add('', 'MVM ou nota fiscal', r.materials.every(item => meaningfulText(item.mvm)), 'recommended', 1, 'Informe MVM ou nota fiscal dos materiais quando disponível.', 'materials');
    add('', 'Número de desenho ou código do vasilhame', r.materials.every(item => meaningfulText(item.desenho) || meaningfulText(item.codigoVasilhame)), 'recommended', 1, 'Informe desenho da peça ou código do vasilhame quando disponível.', 'materials');
    add('', 'Fornecedor dos materiais', r.materials.every(item => meaningfulText(item.fornecedor)), 'recommended', 1, 'Informe o fornecedor quando conhecido.', 'materials');
  }

  add('history-start', 'Início da ocorrência', meaningfulText(h.inicio), 'required', 2, 'Explique como a ocorrência começou.');
  add('history-found', 'Fato identificado', meaningfulText(h.identificado), 'required', 2, 'Descreva o que foi identificado ou constatado.');
  add('history-end', 'Desfecho informado', meaningfulText(h.desfecho), 'required', 2, 'Informe como a situação terminou.');
  add('history-actions', 'Providências verificadas', v.providencias !== 'pending', 'required', 2, 'Confirme se houve providências.');
  if (v.providencias === 'has') {
    add('history-actions', 'Ações adotadas descritas', meaningfulText(h.providenciasFonte) || meaningfulText(h.providencias), 'required', 2, 'Descreva as ações adotadas.');
  }
  if (context.actionsImportant) {
    add('history-called', 'Área ou responsável acionado', meaningfulText(h.acionados), 'recommended', 2, 'Informe as áreas ou responsáveis acionados quando aplicável.');
  }

  add('history-report', 'Relato consolidado e atualizado', meaningfulText(h.relato) && !r.assistant?.stale, 'required', 2, r.assistant?.stale ? 'Os dados foram alterados. Gere ou revise novamente o relato.' : 'Gere ou escreva o relato consolidado.');
  add('', 'Evidências verificadas', v.attachments !== 'pending', 'required', 3, 'Confirme se há fotos ou documentos.', 'attachments');
  if (context.evidenceImportant) {
    add('', 'Foto ou documento da ocorrência', (r.attachments || []).length > 0, 'recommended', 3, 'Inclua evidência quando disponível.', 'attachments');
  }
...
function buildNarrative(record = state.current) {
  ...
  let introduction = `Em ${formatDateOnly(b.data)}, às ${b.hora}, no local ${local || 'informado no boletim'}, foi registrada uma ocorrência${reference ? ` classificada como ${reference}` : ''}`;
  if (meaningfulText(b.nomeEmissor)) introduction += `, comunicada por ${b.nomeEmissor}${b.matriculaEmissor ? `, matrícula ${b.matriculaEmissor}` : ''}`;
  paragraphs.push(`${introduction}.`);

  const factParts = [];
  const start = cleanSentence(h.inicio);
  const found = cleanSentence(h.identificado);
  if (start) factParts.push(`${start}.`);
  if (found && normalizeAssistantText(found) !== normalizeAssistantText(start)) factParts.push(`Foi constatado: ${found}.`);
  if (factParts.length) paragraphs.push(factParts.join(' '));

  if ((record.people || []).length) paragraphs.push(`Pessoas relacionadas: ${joinNatural(record.people.map(assistantPersonSummary))}.`);
  const present = cleanSentence(h.presentes);
  if (present && !(record.people || []).some(person => normalizeAssistantText(present).includes(normalizeAssistantText(person.nome)))) paragraphs.push(`Outras pessoas ou equipes mencionadas: ${present}.`);
  if ((record.vehicles || []).length) paragraphs.push(`Veículos relacionados: ${joinNatural(record.vehicles.map(assistantVehicleSummary))}.`);
  if ((record.materials || []).length) paragraphs.push(`Materiais, peças ou equipamentos relacionados: ${joinNatural(record.materials.map(assistantMaterialSummary))}.`);
  ...
  if (outcome) paragraphs.push(`Ao final, ${outcome}.`);
  const additional = cleanSentence(h.adicional);
  if (additional) paragraphs.push(`Informação complementar: ${additional}.`);
...
function createBlankRecord() {
  ...
  return {
    ...
    finalizedAt: '',
    syncedAt: '',
    operationalTiming: { startedAt:'', endedAt:'', startedBy:null, endedBy:null, autoEnded:false },
...
function normalizeRecord(record) {
  ...
  normalized.schemaVersion = Math.max(5, Number(normalized.schemaVersion || 4));
  normalized.basic ||= {};
  normalized.operationalTiming ||= {};
  normalized.operationalTiming = {
    startedAt: String(normalized.operationalTiming.startedAt || ''),
...
function renderHome() {
  ...
  const attention = state.records.filter(r => r.status === 'Rascunho' && operationalTimeInfo(r).level !== 'ok').length;
  ...
  app.innerHTML = `
    ...
    <div class="metric-grid" aria-label="Resumo dos registros neste dispositivo">
      ...
      <div class="metric-card"><span>Exigem atenção</span><strong>${attention}</strong></div>
    </div>
    ...
    <div class="card-grid">
      ...
      <button class="action-card" data-tone="blue" type="button" data-action="show-handoff">
        <span class="card-icon">${ICONS.list}</span><span><strong>Passagem de turno</strong><span>Veja rascunhos, filas e pendências para o próximo vigilante.</span></span>
      </button>
      <button class="action-card" data-tone="green" type="button" data-action="show-syncqueue">
        <span class="card-icon">${ICONS.sync}</span><span><strong>Fila de sincronização</strong><span>Acompanhe o que ainda precisa chegar à planilha.</span></span>
      </button>
...
function recordCard(record) {
  ...
  const elaboration=elaborationTimeInfo(record);
  const attendance=operationalTimeInfo(record);
  const retry=nextRetryText(record);
  const attendanceChip = attendance.active
    ? `<span class="chip time-chip time-${attendance.level}">Atendimento: ${escapeHtml(attendance.label)}</span>`
    : record.status === 'Rascunho' ? '<span class="chip time-chip time-muted">Atendimento: não iniciado</span>' : '';
  return `<article class="record-card"><div><h3>${escapeHtml(record.numero)}</h3><p>${escapeHtml(summary)}${record.basic?.local?` • ${escapeHtml(record.basic.local)}`:''}</p><div class="entity-meta"><span class="chip status-${record.status.toLowerCase()}">${escapeHtml(record.status)}</span><span class="chip sync-chip sync-${escapeHtml(record.syncStatus||'local')}">${escapeHtml(syncStatusLabel(record))}</span><span class="chip time-chip time-elaboration">Elaboração: ${escapeHtml(elaboration.label)}</span>${attendanceChip}<span class="chip">${source}</span>${retry?`<span class="chip retry-chip">${escapeHtml(retry)}</span>`:''}</div></div><div class="record-side"><time>${formatDateTime(record.updatedAt)}</time><button class="button small secondary" type="button" data-open-record="${record.id}">${record.status==='Rascunho'?'Continuar':'Abrir'}</button></div></article>`;
}
...
async function createNewBo() {
  ...
  addAudit(record, 'CRIAR BOLETIM', 'Novo rascunho criado no aparelho.');
  await dbPut(record);
  await refreshRecords();
...
function renderWizard() {
  ...
  const content = correction
    ...
    : [renderBasicStep, renderRelatedStep, renderHistoryStep, renderAttachmentsStep, renderReviewStep][state.currentStep]();
...
function basicReferenceFields(b) {
  ...
  return `
    <div class="field full">
      ...
      ${referenceDescription ? `<div class="context-help">${ICONS.info}<span>${escapeHtml(referenceDescription)}</span></div>` : '<small>Escolha primeiro o grupo e depois o assunto principal da ocorrência.</small>'}
      <span class="field-error" data-error-for="bo-ref"></span>
    </div>
...
function basicDirectorateFields(b) {
  ...
  return `
    <div class="field full"><label class="required" for="bo-directorate">Diretoria relacionada</label><select id="bo-directorate" data-path="basic.diretoria" data-rerender="true" required>${selectOptions(DIRECTORATES, b.diretoria)}</select><span class="field-error" data-error-for="bo-directorate"></span></div>
...
function renderBasicStep() {
  ...
  return `
    <section class="form-card">
      ...
      <p>Preencha os dados do fato e da pessoa que comunicou a ocorrência à Segurança Patrimonial.</p>
      <div class="form-grid">
        ...
        ${basicRequesterFields(b)}
        ${basicLocationFields(b)}
        ${basicDirectorateFields(b)}
      </div>
    </section>
    ${renderOperationalTimingCard(state.current)}
    <div class="notice info"><strong>Dica:</strong> escolha a referência pelo assunto principal. Pessoas, veículos, materiais e providências serão detalhados nas próximas etapas.</div>`;
}
...
function renderRelatedStep() {
  ...
  <div class="related-grid">
    ${relatedSection('people',{verify:'people',title:'Pessoas',description:'Solicitantes, envolvidos e testemunhas.',button:'Adicionar pessoa',question:'Há pessoa identificada?',noneLabel:'Não há pessoa identificada',empty:'Nenhuma pessoa cadastrada.',render:renderPersonItem,recommended:false})}
...
function reviewItems(record = state.current) {
  ...
  const make = (key, label, step, issues, fallbackTarget, description) => ({
    key,
    label,
    step,
    issues,
    ok: issues.length === 0,
    targetId: issues[0]?.id || fallbackTarget || '',
    targetKey: issues[0]?.key || key,
    description: issues[0]?.message || description
  });

  return [
    make('occurrence', 'Ocorrência preenchida', 0, step0, 'bo-ref', 'Data, referência, solicitante, local e diretoria conferidos.'),
    make('people', 'Pessoas verificadas', 1, step1.filter(issue => issue.key === 'people'), '', 'Pessoas envolvidas foram cadastradas ou confirmadas como não aplicáveis.'),
    make('witnesses', 'Testemunhas verificadas', 1, step1.filter(issue => issue.key === 'witnesses'), '', 'Testemunhas foram cadastradas ou confirmadas como inexistentes.'),
    make('vehicles', 'Veículos verificados', 1, step1.filter(issue => issue.key === 'vehicles'), '', 'Veículos foram cadastrados ou confirmados como não relacionados.'),
    make('materials', 'Materiais verificados', 1, step1.filter(issue => issue.key === 'materials'), '', 'Materiais, peças e cargas foram cadastrados ou confirmados como não relacionados.'),
    make('providencias', 'Providências verificadas', 2, step2.filter(issue => issue.key === 'providencias' || ['history-actions','history-provisions-final'].includes(issue.id)), 'history-actions', 'Ações e áreas acionadas foram conferidas.'),
    make('report', 'Relato consolidado', 2, step2.filter(issue => ['history-start','history-found','history-end','history-report'].includes(issue.id)), 'history-report', 'Relato atualizado e pronto para revisão.'),
    make('attachments', 'Evidências verificadas', 3, step3, '', 'Fotos e documentos foram anexados ou confirmados como indisponíveis.'),
    make('acknowledgements', 'Confirmações finais', 4, step4, 'ack-reviewed', 'Responsabilidade e correspondência dos fatos confirmadas.')
  ];
}
...
function renderReviewStep() {
  ...
  const lockMessage = pending.length ? `Resolva ${pending.length} pendência(s) para finalizar.` : 'Todas as verificações obrigatórias foram concluídas.';

  return `<div class="review-professional-layout">
    <aside class="review-overview-panel">
      <section class="review-summary-card">
        <p class="eyebrow">5. Revisão</p>
        <h1>Revisão do BO</h1>
        <p>${completed.length} de ${items.length} verificações concluídas</p>
        <div class="review-progress"><span style="width:${percent}%"></span></div>
        <div class="review-progress-meta"><strong>${percent}%</strong><span>${pending.length ? `${pending.length} pendência(s) obrigatória(s)` : 'Pronto para finalizar'}</span></div>
      </section>

      ${pending.length ? `<section class="review-status-group pending-group"><div class="review-group-title"><span>${ICONS.warning}</span><div><strong>Pendências</strong><small>Toque em uma linha para abrir somente o que precisa ser corrigido.</small></div></div>${renderReviewStatusList(pending, true)}</section>` : '<section class="review-ready-card">'+ICONS.success+'<div><strong>BO pronto para finalização</strong><small>Todas as informações obrigatórias foram verificadas.</small></div></section>'}

      <section class="review-status-group completed-group"><div class="review-group-title"><span>${ICONS.check}</span><div><strong>Verificações concluídas</strong><small>${completed.length} item(ns) conferido(s)</small></div></div>${renderReviewStatusList(completed)}</section>
    </aside>

    <div class="review-detail-panel">
      <section class="review-context-card"><div><p class="eyebrow">Resumo operacional</p><h2>${escapeHtml(reference || 'Ocorrência não classificada')}</h2><p>${escapeHtml(lockMessage)}</p></div><span class="review-number">${escapeHtml(isOfficialNumber(r.numero) ? r.numero : 'Número gerado ao finalizar')}</span></section>
      <div class="review-grid compact-review-grid">
        ${reviewSection('Dados da ocorrência',0,`<dl class="definition-grid"><div><dt>Data e hora</dt><dd>${formatDateOnly(b.data)} às ${escapeHtml(b.hora)}</dd></div><div><dt>Solicitante</dt><dd>${escapeHtml(b.nomeEmissor)} • ${escapeHtml(b.matriculaEmissor)}</dd></div><div><dt>Local</dt><dd>${escapeHtml(b.local)} — ${escapeHtml(b.complementoLocal)}</dd></div><div><dt>Diretoria</dt><dd>${escapeHtml(directorate || 'Não informada')}</dd></div></dl>`)}
        ${reviewSection('Elementos relacionados',1,`<div class="review-mini-grid"><div><strong>Pessoas</strong><span>${r.people.length || 'Nenhuma'}</span></div><div><strong>Veículos</strong><span>${r.vehicles.length || 'Nenhum'}</span></div><div><strong>Materiais</strong><span>${r.materials.length || 'Nenhum'}</span></div></div>`)}
      ...
      <div class="notice info"><strong>Número oficial:</strong> será atribuído pelo Google Sheets após a finalização e sincronização do boletim.</div>
    </div>
  </div>`;
}
...
async function handleStepAction(action) {
  ...
  if (action === 'finalize') {
    ...
    const confirmed = await openAppModal({
      ...
      details:'<ul><li>Confira relato e providências.</li><li>Confirme pessoas, veículos, materiais e anexos.</li><li>Não feche o aplicativo durante o envio.</li><li>Se houver atendimento operacional em andamento, ele será encerrado no horário da finalização.</li></ul>',
    ...
    try {
      ...
      state.current.operationalTiming ||= { startedAt:'', endedAt:'', startedBy:null, endedBy:null, autoEnded:false };
      if (state.current.operationalTiming.startedAt && !state.current.operationalTiming.endedAt) {
        state.current.operationalTiming.endedAt = state.current.finalizedAt;
        state.current.operationalTiming.endedBy = operatorSnapshot();
        state.current.operationalTiming.autoEnded = true;
        addAudit(state.current, 'ENCERRAR ATENDIMENTO', 'Atendimento operacional encerrado automaticamente no momento da finalização do BO.');
      }
...
function collectStepIssues(step, record = state.current) {
  ...
  if (step === 0) {
    ...
    if (isOtherReference(b.referencia) && !String(b.referenciaOutra || '').trim()) add('bo-ref-other','Descreva a referência não listada.');
  ...
  if (step === 1) {
    ...
    const context = assistantContext(record);
    if (context.vehicleRequired && !record.vehicles.length) add('', 'Esta ocorrência exige o cadastro do veículo relacionado.','Informação importante ausente','vehicles');
    if (context.materialRequired && !record.materials.length) add('', 'Esta ocorrência exige o cadastro do material, peça ou equipamento relacionado.','Informação importante ausente','materials');
  }
  ...
  if (step === 2) {
    ...
    if (record.verification.providencias === 'has' && !String(h.providenciasFonte || h.providencias || '').trim()) add('history-actions','Descreva as providências adotadas.');
    if (record.verification.providencias === 'has' && !String(h.providencias || '').trim()) add('history-provisions-final','Gere ou escreva as providências consolidadas.');
    if (!String(h.relato || '').trim()) add('history-report','Gere ou escreva o relato consolidado.');
    if (record.assistant?.stale && String(h.relato || '').trim()) add('history-report','Os dados foram alterados depois da geração. Atualize ou revise o relato.');
  }
...
function renderDetail() {
  ...
  const reference=resolvedReference(b);
  const directorate=b.diretoria==='Outra'?b.diretoriaOutra:b.diretoria;
  const elaboration=elaborationTimeInfo(r), attendance=operationalTimeInfo(r);
  ...
  <div class="review-grid">
    ${reviewSectionStatic('Ocorrência',`<dl class="definition-grid"><div><dt>Referência</dt><dd>${escapeHtml(reference||'Não informada')}</dd></div><div><dt>Local</dt><dd>${escapeHtml(b.local)} — ${escapeHtml(b.complementoLocal)}</dd></div><div><dt>Diretoria</dt><dd>${escapeHtml(directorate||'Não informada')}</dd></div><div><dt>Solicitante</dt><dd>${escapeHtml(b.nomeEmissor)} • ${escapeHtml(b.matriculaEmissor)}</dd></div><div><dt>Ocorrência informada</dt><dd>${formatDateOnly(b.data)} às ${escapeHtml(b.hora)}</dd></div><div><dt>BO criado</dt><dd>${formatDateTime(r.createdAt)}</dd></div><div><dt>BO finalizado</dt><dd>${formatDateTime(r.finalizedAt)}</dd></div><div><dt>Tempo de elaboração</dt><dd>${escapeHtml(elaboration.label)}</dd></div><div><dt>Tempo de atendimento</dt><dd>${attendance.active ? escapeHtml(attendance.label) : 'Não registrado'}</dd></div></dl>`)}
