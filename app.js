...
async function syncRecord(record, notify = true) {
  ...
  try {
    ...
    // A gravação pode ter sido concluída no Google Sheets e apenas a resposta ter se perdido.
    // Antes de marcar erro, consultamos o mesmo ID no servidor. Isso impede BO duplicado.
    const recovered = await recoverRecordFromServer(record);
    if (recovered) {
      if (notify) showToast(`Boletim ${record.numero} confirmado pela planilha.`);
      return true;
    }
    ...
    if (state.current?.id === record.id) state.current = structuredClone(record);
    updateHeader();
    if (notify) await openAppModal({ kind:'danger', eyebrow:'Falha de sincronização', title:'Não foi possível confirmar o envio', message:escapeHtml(error.message), details:'O registro permanece salvo no aparelho. Ao tentar novamente, o servidor reutilizará o mesmo número oficial para este ID, evitando boletins duplicados.', confirmText:'Entendi' });
    return false;
  }
}
