(() => {
  ...
  const NEXO_CONTEXT_KEY = 'bo-digital-nexo-context-session-v31';
  ...
  function attentionRecords(records) {
    const score = record => {
      ...
      if (sla?.level === 'critical') value += 650;
      else if (sla?.level === 'warning') value += 350;
  ...
  function currentSituation(records) {
    ...
    const pendingSync = records.filter(record => ['error', 'pending', 'conflict'].includes(record.syncStatus));
  ...
  function compactContext(records) {
    ...
    return {
      ...
      totais: {
        ...
        syncPendente: records.filter(r => ['pending', 'error', 'conflict'].includes(r.syncStatus)).length
      },
      focoAtual: {
  ...
  async function handleLocalCommand(text, records) {
    ...
    if (syncActionRequested) {
      ...
      if (/\b(pendentes|pendente|todos|tudo|sem sincronizar|nao sincronizados)\b/.test(n)) {
        const targets = records.filter(record => ['pending', 'error', 'local', 'conflict'].includes(record.syncStatus) && record._source !== 'remote');
    ...
    if (/\b(ajuda|o que voce faz|comandos|capaz)\b/.test(n)) {
      return { handled: true, response: 'Posso mostrar a situação atual, apontar prioridades, listar rascunhos e pendências de sincronização, conferir o checklist do BO, abrir a passagem de turno, a fila e o diagnóstico, localizar/abrir/resumir BOs, sincronizar com confirmação e iniciar um novo boletim. Diga também “modo conversa” para usar voz de forma contínua.' };
    }
    ...
    if (/\b(sem sincronizar|sincronizacao|sincronizar)\b/.test(n) && /\b(quais|lista|mostr|quant|pendente|erro)\w*\b/.test(n)) {
      const pendingSync = records.filter(record => ['pending', 'error', 'local', 'conflict'].includes(record.syncStatus) && record._source !== 'remote');
