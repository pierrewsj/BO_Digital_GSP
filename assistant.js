(() => {
  ...
  const NEXO_CONTEXT_KEY = 'bo-digital-nexo-context-session-v32';
  ...
  if (!messages.length) {
    messages = [{
      role: 'assistant',
      text: 'Olá. Eu sou o NEXO Operador. Além de localizar e acompanhar BOs, posso orientar o roteiro padronizado de cada ocorrência, conferir pendências e ajudar a evitar informações esquecidas. Diga “situação atual”, “roteiro deste BO” ou “checklist deste BO”.',
      ts: Date.now()
    }];
    saveHistory();
  }
  ...
  async function handleLocalCommand(text, records) {
    ...
    if (/\b(ajuda|o que voce faz|comandos|capaz)\b/.test(n)) {
      return { handled: true, response: 'Posso mostrar a situação atual, orientar o roteiro padronizado do BO, conferir o checklist e pendências, apontar prioridades, listar rascunhos, abrir a passagem de turno, a fila e o diagnóstico, localizar/abrir/resumir BOs, sincronizar com confirmação e iniciar um novo boletim. Diga “roteiro deste BO”, “checklist deste BO” ou “modo conversa”.' };
    }
