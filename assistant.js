(() => {
  ...
  const NEXO_HISTORY_KEY = 'bo-digital-nexo-history-v30';
  ...
  const NEXO_CONTEXT_KEY = 'bo-digital-nexo-context-session-v30';
  ...
  if (!messages.length) {
    messages = [{
      role: 'assistant',
      text: 'Olá. Eu sou o NEXO Operador. Posso manter o contexto da conversa, apontar prioridades, localizar e abrir BOs, listar pendências e sincronizar registros com confirmação. Diga “situação atual” para começar.',
      ts: Date.now()
    }];
    saveHistory();
  }
  ...
  function detailedRecordSummary(record) {
    ...
    const related = `${(record.people || []).length} pessoa(s), ${(record.vehicles || []).length} veículo(s) e ${(record.materials || []).length} material(is)`;
    ...
    return [
      ...
      `Referência: ${recordReference(record)}. Local: ${local}. Solicitante: ${safeText(b.nomeEmissor || 'não informado')}.`,
  ...
  function attentionRecords(records) {
    const score = record => {
      let value = 0;
      if (record.syncStatus === 'error') value += 1000;
      if (record.syncStatus === 'pending') value += 700;
      if (record.status === 'Rascunho') value += 300;
      value += Math.min(240, Math.floor(ageMs(record) / 3600000) * 8);
      return value;
    };
    return records.filter(record => record.status === 'Rascunho' || ['error', 'pending'].includes(record.syncStatus)).sort((a, b) => score(b) - score(a));
  }
  ...
  function currentSituation(records) {
    ...
    const pendingSync = records.filter(record => ['error', 'pending'].includes(record.syncStatus));
  ...
  function compactContext(records) {
    ...
    return {
      ...
      totais: {
        ...
        syncPendente: records.filter(r => ['pending', 'error'].includes(r.syncStatus)).length
      },
      focoAtual: {
  ...
  async function handleLocalCommand(text, records) {
    ...
    if (syncActionRequested) {
      ...
      if (/\b(pendentes|pendente|todos|tudo|sem sincronizar|nao sincronizados)\b/.test(n)) {
        const targets = records.filter(record => ['pending', 'error', 'local'].includes(record.syncStatus) && record._source !== 'remote');
    ...
    if (/\b(prioridade|prioridades|atencao|precisa de atencao|pendencias principais)\b/.test(n)) {
      ...
      return { handled: true, response: attention.length
        ? `Prioridades encontradas:\n${listText(attention, '', 5)}\nConsiderei primeiro falhas/pendências de sincronização e depois rascunhos mais antigos.`
    ...
    if (/\b(ajuda|o que voce faz|comandos|capaz)\b/.test(n)) {
      return { handled: true, response: 'Posso mostrar a situação atual, apontar prioridades, listar rascunhos e pendências de sincronização, localizar/abrir/resumir BOs, lembrar o registro citado na conversa, sincronizar com confirmação e iniciar um novo boletim. Diga também “modo conversa” para usar voz de forma contínua.' };
    }
    ...
    if (/\b(sem sincronizar|sincronizacao|sincronizar)\b/.test(n) && /\b(quais|lista|mostr|quant|pendente|erro)\w*\b/.test(n)) {
      const pendingSync = records.filter(record => ['pending', 'error', 'local'].includes(record.syncStatus) && record._source !== 'remote');
  ...
  async function submitPrompt(rawText) {
    ...
    try {
      ...
      addMessage('assistant', 'Não reconheci esse pedido no modo local. Tente “situação atual”, “prioridades”, “quais rascunhos existem?”, “resumir BO 26”, “abrir BO 26” ou “sincronizar pendentes”. Para perguntas totalmente livres, ative a integração de IA no Apps Script.', { speak: false });
    } finally {
      setBusy(false);
      updateStatus();
